...
async def test_profile_by_id_resource(mcp: FastMCP, mock_service: AsyncMock) -> None:
    mock_service.get_profile.return_value = Profile(id="p1", title="Morning Brew", ratio=16.0)
...
async def test_profile_by_id_not_found(mcp: FastMCP, mock_service: AsyncMock) -> None:
    mock_service.get_profile.side_effect = NotFoundError(
...
async def test_create_profile_from_link(mcp: FastMCP, mock_service: AsyncMock) -> None:
    mock_service.create_profile_from_link.return_value = Profile(id="p3", title="Imported Profile")
...
async def test_create_profile_from_fields(mcp: FastMCP, mock_service: AsyncMock) -> None:
    mock_service.create_profile.return_value = Profile(id="p4", title="Manual Profile", ratio=15.5)
...
async def test_delete_profile_unavailable(mcp: FastMCP, mock_service: AsyncMock) -> None:
    mock_service.delete_profile.side_effect = CloudUnreachableError(
    ...
    assert error_body["error"]["code"] == "cloud_unreachable"


async def test_generate_profile_link_success(mcp: FastMCP, mock_service: AsyncMock) -> None:
    mock_service.generate_link.return_value = ProfileLink(url="https://fellow.app/p/abc123")
